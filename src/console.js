...
$(document).mousemove(function (event) {
  ...
  //支持按下鼠标， 框选多个node
  if (KFK.mode === "pointer" && KFK.mouseIsDown && (KFK.lineMoverDragging=== false)) {
    KFK.warn(
  ...
  $("#linetransformer").draggable({
    ...
    stop: () => {
      ...
      if (KFK.tobeTransformJqLine) {
        ...
        KFK.syncNodePut("D", KFK.tobeTransformJqLine, " delete old line", null, false);
...
KFK.setTipBkgColor = function (theNode, bgColor) {
  ...
  if (ret) KFK.syncNodePut("U", $(theNode), "change bkg color", KFK.fromJQ, false);
};
  ...
  $(KFK.C3)
    ...
    .each((index, aLinkDiv) => {
      ...
      if (
        ...
        KFK.syncNodePut("D", $(aLineDiv), "delete line for deleting node", null, false);
        $(aLinkDiv).remove();
      }
    });
...
KFK.removeTipLink = function (from, to) {
  ...
  for (var i = 0; i < KFK.tipLinks.length; i++) {
    if (
      (from === null && KFK.tipLinks[i].to === to) ||
...
KFK.onWsMsg = function (data) {
  ...
  switch (data.payload.cmd) {
    ...
    case "TGLREAD":
      ...
      if (data.payload.doc_id === KFK.APP.model.cocodoc.doc_id) {
        ...
        localStorage.setItem(
          "cocodoc",
          JSON.stringify(KFK.APP.model.cocodoc.doc_id)
        );
      }
      break;
    case "NEWPRJ":
    ...
    case "ZI":
      ...
      break;
    case "LOCKNODE":
      KFK.NodeController.lock(
        $(`#${data.payload.data.nodeid}`),
        KFK.putOpActionEntry
      );
      break;
    case "UNLOCKNODE":
  ...
  let spinnerLineWidth = $("#spinner_line_width").spinner({
    ...
    spin: function (event, ui) {
      if (KFK.pickedJqLine != null && KFK.docLocked() === false) {
        ...
        KFK.syncNodePut("U", KFK.pickedJqLine, "set line width", KFK.fromJQ, false);
      }
    }
  });
  ...
  $("#lineColor").spectrum({
    ...
    change: function (color) {
      ...
      if (KFK.pickedJqLine != null && KFK.docLocked() === false) {
        ...
        options.color = rgb;
        KFK.setLineOptions(KFK.pickedJqLine, options);
        KFK.syncNodePut("U", KFK.pickedJqLine, "set line color", KFK.fromJQ, false);
      }
    }
  });
  $("#fontColor").spectrum({
  ...
  $("#tipBkgColor").spectrum({
    ...
    change: function (color) {
      ...
      if (theNode != null && KFK.notAnyLocked($(theNode))) {
        ...
        KFK.syncNodePut("U", $(theNode), "set tip bkg color", KFK.fromJQ, false);
      }
    }
  });
};
...
KFK.addContainerMainEventHandler = function () {
  ...
  $(document).keydown(function (e) {
    ...
    if (e.keyCode === 16) {
      ...
      KFK.setMode("pointer");
    } else if (e.keyCode === 46 || e.keyCode === 8 || e.keyCode === 88) {
      // key DELETE  key X
      preventDefault = true;
    ...
    } else if (e.keyCode === 35 && KFK.isKfkNode(KFK.jqHoverDIV)) {
      // key end
      preventDefault = true;
    ...
    } else if (e.keyCode === 36 && KFK.isKfkNode(KFK.jqHoverDIV)) {
      // key home
      preventDefault = true;
      let myZI = KFK.getZIndex(KFK.jqHoverDIV);
      let count = 0;
    ...
    } else if (e.keyCode === 33 && KFK.isKfkNode(KFK.jqHoverDIV)) {
      // key pageup
      preventDefault = true;
      let myZI = KFK.getZIndex(KFK.jqHoverDIV);
      let count = 0;
    ...
    } else if (e.keyCode === 34 && KFK.isKfkNode(KFK.jqHoverDIV)) {
      // key pagedown
      preventDefault = true;
      let zIndexChanger = { doc_id: KFK.APP.model.cocodoc.doc_id, ZI: {} };
      let myZI = KFK.getZIndex(KFK.jqHoverDIV);
      ...
      if (KFK.jqHoverDIV && KFK.isMyDoc() && KFK.docLocked() === false) {
        ...
        KFK.log(
          "Not authed" +
            KFK.APP.model.cocodoc.owner +
            " " +
            KFK.APP.model.cocouser.userid
        );
        console.log(KFK.APP.model.cocodoc);
      }
    } else if (
      (e.keyCode === 83 || e.keyCode === 87 || e.keyCode === 81) &&
      e.metaKey
    ) {
...
KFK.addTextToHoverDIV = function (text) {
  if (KFK.jqHoverDIV) {
    if (config.node[KFK.jqHoverDIV.attr("nodetype")].edittable) {
      ...
      innerObj.html(newText);
      KFK.syncNodePut("U", KFK.jqHoverDIV, "add text to hover div", KFK.fromJQ, false);
    }
  }
};
...
KFK.navActionLog = function (item, direction) {
  ...
  if(direction === 'first'){ //go left
    item.pos = 0;
  }else if(direction === 'prev'){ //go left
    item.pos = item.pos -1;
    if(item.pos < 0){
