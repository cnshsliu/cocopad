...
KFK.undo = () => {
    ...
    if (ope.cmd === 'LOCK') {
        KFK.sendCmd('UNLOCKNODE', { doc_id: KFK.APP.model.cocodoc.doc_id, nodeid: ope.from });
        ...
        if (ope.from !== '') {
            ...
            if (jqFrom.hasClass('kfknode')) {
                ...
                jqFrom.addClass('liukehong1');
            }
            if (jqFrom.hasClass('lock')) {
                x(nodeid, 'hasclass lock, disable draggable');
                jqFrom.draggable('disable');
                jqFrom.resizable('disable');
                jqFrom.droppable('disable');
            } else {
...
KFK.onWsMsg = function (data) {
    ...
    switch (data.payload.cmd) {
        ...
        case 'ZI':
            ...
            break;
        case 'LOCKNODE':
            KFK.NodeController.lock($(`#${data.payload.data.nodeid}`), KFK.putOpActionEntry);
            break;
        case 'UNLOCKNODE':
...
KFK.addContainerMainEventHandler = function () {
    ...
    $(document).keydown(function (e) {
        ...
        if (e.keyCode === 16) { //Shift
            ...
            KFK.toggleRight();
        } else if (e.keyCode === 17) { //Ctrl key ctrl
            KFK.ctrlDown = true;
...
KFK.viewActionLog = function (index) {
    ...
    KFK.showTip(`Shift + 上下键 查看${KFK.APP.model.actionlog[index].editor}参与过的节点`);
    KFK.actionLogToView = KFK.APP.model.actionlog[index];
    KFK.actionLogToViewIndex = 0;
    KFK.actionLogGoto(0);
};
KFK.actionLogGoto = function (index) {
    if (index < 0 || index > KFK.actionLogToView.actionlog.length - 1)
        return;
    let nodeid = KFK.actionLogToView.actionlog[index];
    console.log('actionlog ', index, nodeid);
...
KFK.actionLogNext = function () {
    if (KFK.actionLogToViewIndex < KFK.actionLogToView.actionlog.length - 1) {
        console.log('goto next');
        KFK.actionLogToViewIndex += 1;
        KFK.actionLogGoto(KFK.actionLogToViewIndex);
        // } else {
        //     KFK.showWarning('到底了');
    }
};
KFK.actionLogPrevious = function () {
    console.log(KFK.actionLogToViewIndex);
    if (KFK.actionLogToViewIndex > 0) {
...
//TODO: event handler after undo/redo
//TODO: action log is not shown
//TODO: 清理OSS图片
