...
export default class RtcClient {
  ...
  async join() {
    ...
    try {
      ...
      this.localStream_ = TRTC.createStream({
        ...
        video: true,
        userId: this.userId_,
        cameraId: RtcCommon.getCameraId(),
        microphoneId: RtcCommon.getMicrophoneId(),
        mirror: true
      });
      try {
        ...
        $('.main-video-btns').removeClass('noshow');
    ...
    for (let state of states) {
      if (state.audioMuted) {
        $('#' + state.userId)
          ...
          .attr('src', KFK.getFrontEndUrl('rtc/mic-off.png'));
      }
      if (state.videoMuted) {
        $('#' + state.userId)
          .find('.member-video-btn')
          .attr('src', KFK.getFrontEndUrl('rtc/camera-off.png'));
        // $('#mask_' + this.members_.get(state.userId).getId()).removeClass('noshow');
      }
    }
  }
  ...
  handleEvents() {
    ...
    this.client_.on('stream-added', evt => {
      ...
      if (remoteStream.getUserId() === RtcCommon.shareUserId) {
    ...
    this.client_.on('stream-subscribed', evt => {
      ...
      remoteStream.on('player-state-changed', event => {
        ...
        if (event.type == 'video' && event.state == 'STOPPED') {
          // $('#mask_' + remoteStream.getId()).addClass('noshow');
          $('#' + remoteStream.getUserId())
            .find('.member-video-btn')
            .attr('src', KFK.getFrontEndUrl('rtc/camera-off.png'));
        }
        if (event.type == 'video' && event.state == 'PLAYING') {
          // $('#mask_' + remoteStream.getId()).removeClass('noshow');
          $('#' + remoteStream.getUserId())
            .find('.member-video-btn')
      ...
      //   // mask.removeClass('noshow');
      //   $('#' + remoteStream.getUserId())
      //     .find('.member-video-btn')
      //     .attr('src', KFK.getFrontEndUrl('rtc/camera-off.png'));
      // }
      // mask.addClass('noshow');
    ...
    this.client_.on('unmute-audio', evt => {
      ...
      $('#' + evt.userId)
        .find('.member-audio-btn')
        .attr('src', KFK.getFrontEndUrl('rtc/mic-on.png'));
    });
    this.client_.on('mute-video', evt => {
      ...
      // if (streamId) {
      //   $('#mask_' + streamId).removeClass('noshow');
      // }
    });
    this.client_.on('unmute-video', evt => {
      ...
      if (stream) {
        ...
        // if (streamId) {
        //   $('#mask_' + streamId).addClass('noshow');
        // }
      }
    });
  }
